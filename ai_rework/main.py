...
if __name__ == "__main__":
    ...
    while True: # Loop until a profile is set or user quits
        if profiles_data:
            ...
            action = input("Wybierz profil, utwórz nowy, lub zakończ (wpisz numer, 'nowy', 'koniec'): ").strip().lower()
            ...
            if action.isdigit():
                try:
                    ...
                    print("Nieprawidłowe dane wejściowe.") # Should be caught by isdigit already
            elif action == 'nowy':
        ...
        if not active_profile_path: # Check if profile not yet selected
            if not profiles_data:
                ...
                create_choice = input("Czy chcesz utworzyć nowy profil? (tak/nie): ").strip().lower()
            ...
            if create_profile(new_profile_name_input):
                ...
                for p_name in profiles_data.keys():
                    ...
                    if new_profile_name_input.lower() == p_name.lower() or \
                       new_profile_name_input.replace(" ", "_") == p_name.replace(" ", "_"): # Basic heuristic
                        created_profile_actual_name = p_name
                        break
                
                if not created_profile_actual_name and new_profile_name_input in profiles_data:
                ...
                if created_profile_actual_name:
                    ...
                    if path:
                        ...
                        print(f"Błąd: Nie udało się automatycznie wybrać nowo utworzonego profilu '{created_profile_actual_name}'. Spróbuj wybrać ręcznie.")
                    ...
                    print(f"Profil mógł zostać utworzony z inną nazwą z powodu sanitacji. Sprawdź listę.")

            else:
                # Creation failed, message printed by create_profile
                try_again = input("Czy chcesz spróbować utworzyć profil ponownie? (tak/nie): ").strip().lower()
                if try_again != 'tak':
                    print("Zamykanie gry.")
                    exit()
                # else loop continues
    ...
    if active_profile_path and active_profile_name:
        ...
        if loaded_player:
            ...
            while True:
                choice = input("Czy chcesz kontynuować (k) poprzednią grę, czy rozpocząć nową (n)? (k/n): ").strip().lower()
                if choice == 'k':
            ...
            print("Brak zapisanego stanu gry dla tego profilu. Rozpoczynanie nowej gry...")
            player_instance_to_start = PlayerCharacter(is_new_character=True)
