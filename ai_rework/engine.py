...
def perform_sss(player_character: 'PlayerCharacter'):
    ...
    if player_character.stats["luck"] <= 0:
        print(
            "Twoje SZCZĘŚCIE jest na zerowym poziomie lub poniżej! Automatyczna porażka w teście SSS."
        )
    ...
    print(
        f"Twój poziom SZCZĘŚCIA (przed rzutem, po odjęciu 1 za próbę): {player_character.stats['luck']}"
    )
    ...
    if dice_roll <= current_luck_val:
        print(
            f"Wynik rzutu ({dice_roll}) <= Poziom SZCZĘŚCIA przed próbą ({current_luck_val}). Udało się! Masz SZCZĘŚCIE!"
        )
        print("-----------------------------")
        return True
    else:
        print(
            f"Wynik rzutu ({dice_roll}) > Poziom SZCZĘŚCIA przed próbą ({current_luck_val}). Niestety, nie masz SZCZĘŚCIA."
        )
        print("-----------------------------")
        return False
    ...
    for action in paragraph_actions:
        ...
        if isinstance(details, str):
            ...
            if action_type == "stat_change":
                match = re.match(
                    r"(lose|tracisz|gain|zyskujesz|otrzymujesz|dostajesz|\+|-)\s*(\d+)\s*([a-zA-ZĄĆĘŁŃÓŚŹŻ\s]+)",
                    details,
                    re.IGNORECASE,
                )
                if match:
                    ...
                    temp_details_dict = {
                        "stat": stat_key,
                        "amount": amount,
                        "original_detail": parsed_details_str,
                    }
                else:  # Fallback if regex fails
                    temp_details_dict = {
                        "original_detail": parsed_details_str
                    }  # Store original to avoid losing info
            elif action_type in [
                "get_item",
                "item_gain",
                "lose_item",
            ...
            print(
                f"  Typ: {action_type.replace('_', ' ').capitalize()} (info: \"{parsed_details_str}\")"
            )
        else:
            print(
                f"  Typ: {action_type.replace('_', ' ').capitalize()} (detale: {details})"
            )
        ...
        if action_type == "stat_change":
            ...
            if "złot" in item_name.lower():  # e.g. "10 sztuk złota" or "złoto"
                ...
                required_gold = (
                    int(gold_amount_match.group(1)) if gold_amount_match else 1
                )  # Assume 1 if no number
            ...
            if (
                not attribute_from_json
                or threshold is None
                or success_paragraph_id is None
                or failure_paragraph_id is None
            ):
            ...
            if player_value is None:
                print(
                    f"    Błąd w ATTRIBUTE_TEST: nieznany atrybut gracza '{attribute}'."
                )
                continue

            print(
                f"    Test Atrybutu: Twoja {attribute.capitalize()} ({player_value}) vs Próg ({threshold})."
            )
            ...
            if next_paragraph_id is None:
                next_paragraph_id = outcomes.get(roll_result)
            if next_paragraph_id is None:
                next_paragraph_id = outcomes.get("default")
            ...
            if next_paragraph_id is not None:
                ...
                print(
                    "    Wynik rzutu nie prowadzi do zmiany paragrafu (brak pasującej definicji w 'outcomes')."
                )

        elif action_type == "dice_roll":
            dice_count = 1
            detail_text_for_dice = details.get(
                "original_detail", str(details)
            )  # Use original string if parsed
            if "dwiema kostkami" in detail_text_for_dice:
                dice_count = 2
            roll_result = roll_k6(dice_count)
            print(
                f"    Rzut {dice_count}K6: Wynik = {roll_result}. (Tekst paragrafu wyjaśni znaczenie)"
            )
            ...
            if current_paragraph_data and current_paragraph_data.get("monster"):
                ...
                print(
                    f"    Uwaga: W tym miejscu jest {current_paragraph_data['monster']['name']}!"
                )
                # Combat itself is initiated in start_game loop based on monster presence
            else:
                print(
                    "    Błąd: Akcja monster_encounter, ale brak danych potwora lub kontekstu paragrafu."
                )

        if player.stats["stamina"] <= 0:
            print(
                f"    Twoja Wytrzymałość spadła do {player.stats['stamina']} lub poniżej. Umierasz!"
            )
...
def initiate_combat(
    ...
    while True:
        ...
        print(
            f"Siła Ataku Potwora ({monster['name']}): {monster_attack_strength} (2K6 + Z:{monster['dexterity']})"
        )

        player_attack_strength = (
            roll_k6(2) + player_character.stats["dexterity"] + temp_player_dex_bonus
        )
        print(
            f"Twoja Siła Ataku: {player_attack_strength} (2K6 + Z:{player_character.stats['dexterity']})"
        )

        if monster_attack_strength > player_attack_strength:
            print(
                f"Potwór ({monster_attack_strength}) trafia Ciebie ({player_attack_strength})!"
            )
            damage_taken_initial = 2

            sss_choice = (
                input("  Otrzymałeś obrażenia. Użyć SZCZĘŚCIA (SSS)? (tak/nie): ")
                .strip()
                .lower()
            )
            ...
            if sss_choice == "tak" or sss_choice == "t":
                ...
                if sss_result:
                    ...
                    print(
                        "  Dzięki SZCZĘŚCIU, tracisz tylko 1 punkt Wytrzymałości w tej rundzie!"
                    )
                else:
                    actual_damage_taken = 3
                    print(
                        "  Pech! Tracisz dodatkowo 1 punkt Wytrzymałości (łącznie 3 punkty) w tej rundzie!"
                    )
            else:
            ...
            print(
                f"Ty ({player_attack_strength}) trafiasz potwora ({monster_attack_strength})!"
            )
            damage_dealt_initial = 2

            sss_choice = (
                input("  Zadałeś obrażenia. Użyć SZCZĘŚCIA (SSS)? (tak/nie): ")
                .strip()
                .lower()
            )
            ...
            if sss_choice == "tak" or sss_choice == "t":
                ...
                if sss_result:
                    ...
                    print(
                        f"  Dzięki SZCZĘŚCIU, {monster['name']} traci dodatkowo 2 punkty Wytrzymałości (łącznie 4 punkty)!"
                    )
                else:
                    actual_damage_dealt = 1
                    print(
                        f"  Pech! {monster['name']} traci tylko 1 punkt Wytrzymałości w tej rundzie."
                    )
            else:
                print(
                    f"  {monster['name']} traci {actual_damage_dealt} punkty Wytrzymałości."
                )
            ...
            print(
                f"Twoja Wytrzymałość: {player_character.stats['stamina']}, Wytrzymałość Potwora ({monster['name']}): {monster['stamina']}"
            )
            time.sleep(1)
            continue
        ...
        for choice_data in current_paragraph_data.get("choices", []):
            ...
            if "uciek" in choice_text and (
                "patrz" in choice_text
                or "idź do" in choice_text
                or "ucieczk" in choice_text
            ):
        ...
        if flee_choice_info:
            flee_prompt = (
                input(
                    f"  Możesz spróbować ucieczki (tekst wyboru: \"{flee_choice_info['text']}\"). Uciec? (tak/nie): "
                )
                .strip()
                .lower()
            )
            if flee_prompt == "tak" or flee_prompt == "t":
                penalty = 2
                print(
                    f"Uciekasz! Tracisz {penalty} punkty Wytrzymałości za próbę ucieczki."
                )
                player_character.change_stat("stamina", -penalty)
                if player_character.stats["stamina"] <= 0:
                    return "defeat"
                player_character.current_paragraph_id = flee_choice_info[
                    "target_paragraph_id"
                ]
                return "fled"
...
def start_game(active_profile_path: str, player: 'PlayerCharacter'): # Added player parameter
    if not active_profile_path or not os.path.isdir(active_profile_path):
        print(f"Błąd krytyczny: Ścieżka profilu '{active_profile_path}' jest nieprawidłowa lub nie istnieje. Nie można uruchomić gry.")
        return
    ...
    while True:
        current_paragraph_data = get_paragraph(
            player.current_paragraph_id, game_paragraphs
        )

        if not current_paragraph_data:
            print(
                f"BŁĄD KRYTYCZNY: Nie znaleziono paragrafu o ID {player.current_paragraph_id}! Kończenie gry."
            )
            break
        ...
        if current_paragraph_data.get("monster"):
            ...
            combat_prompt = (
                input(
                    "Czy chcesz zaatakować? (tak/nie, 'nie' oznacza próbę zignorowania jeśli możliwe): "
                )
                .strip()
                .lower()
            )
            if combat_prompt == "tak" or combat_prompt == "t":
                combat_result = initiate_combat(
                    player,
                    current_paragraph_data["monster"],
                ...
                if combat_result == "defeat":
                    ...
                    if player.stats["stamina"] > 0:
                        print(
                            "\nZOSTAŁEŚ POKONANY!"
                        )  # if not already dead by stamina loss
                    print("--- KONIEC GRY ---")
                    break
                    ...
                    print(
                        f"Pokonałeś {current_paragraph_data['monster']['name']}! Rozglądasz się co dalej..."
                    )
            else:
                print(
                    f"Decydujesz się nie atakować {current_paragraph_data['monster']['name']} w tym momencie."
                )

        (
            action_taken_by_event,
            game_should_end,
            paragraph_allows_eating_current_turn,
        ) = handle_actions(
            player,
            current_paragraph_data.get("actions", []),
            current_paragraph_data,
            game_paragraphs,
        )
        ...
        if game_should_end:
            ...
            if (
                player.stats["stamina"] > 0 and not action_taken_by_event
            ):  # If not dead by stamina and action didn't specify exact end
        ...
        for i, choice_data in enumerate(actual_choices_for_display):
            ...
            target_id = choice_data.get("target_paragraph_id") # Ensure target_id is fetched safely

            # New condition: Check for "-patrz XYZ" or "patrz XYZ" as a redirection instruction
            # and ensure target_id exists.
            if target_id is not None and re.search(r"(-?\s*patrz\s+\d+)", choice_text, re.IGNORECASE):
            ...
            # Original condition: If choice text is just a number, or seems like a direct reference like "patrz 123" by itself.
                ...
                or (
                    choice_text.lower().startswith("patrz")
                    and choice_text.replace("patrz", "").strip().isdigit()
                )
            ):
                display_text = f"Idź do paragrafu {target_id}"
            else:
                ...
                if not temp_display_text and target_id is not None: # If stripping "patrz" left nothing, but it's a valid choice
                    display_text = f"Przejdź do paragrafu {target_id}"
                elif not temp_display_text and target_id is None: # Should not happen with valid choices
        ...
        if paragraph_allows_eating_current_turn:
            general_actions_menu[current_choice_offset] = {
                "text": "Zjedz prowiant",
                "action": "eat_provision",
            }
            print(f"  {current_choice_offset}. Zjedz prowiant")
            current_choice_offset += 1

        general_actions_menu[current_choice_offset] = {
            "text": "Pokaż statystyki ponownie",
        ...
        while True:
            try:
                ...
                if 1 <= choice_num <= num_paragraph_choices:
                    ...
                    cls_term()
                    chosen_general_action_key = general_actions_menu[choice_num][
                        "action"
                    ]
                    ...
                    if chosen_general_action_key == "use_elixir":
                        ...
                        if paragraph_allows_eating_current_turn:
                            ...
                            print(
                                "W tym momencie nie możesz zjeść prowiantu (tekst paragrafu na to nie zezwala)."
                            )
                    ...
                    for i, choice_data_rep in enumerate(actual_choices_for_display):
                        ...
                        target_id_rep = choice_data_rep.get("target_paragraph_id") # Safe fetch

                        # Apply the same logic as above for re-listing choices
                        if target_id_rep is not None and re.search(r"(-?\s*patrz\s+\d+)", choice_text_rep, re.IGNORECASE):
                            match_rep = re.search(r"(-?\s*patrz\s+(\d+))", choice_text_rep, re.IGNORECASE)
                            if match_rep:
                                display_text_rep = f"Idź do paragrafu {target_id_rep}"
                            else:
                                display_text_rep = f"Przejdź do paragrafu {target_id_rep}"
                        elif target_id_rep is not None and (
                            not choice_text_rep
                            or choice_text_rep.isdigit()
                            or (
                                choice_text_rep.lower().startswith("patrz")
                                and choice_text_rep.replace("patrz", "").strip().isdigit()
                            )
                        ):
                            display_text_rep = f"Idź do paragrafu {target_id_rep}"
                        else:
                    ...
                    for gm_key_display in sorted(general_actions_menu.keys()):
                        if gm_key_display >= temp_offset_display:
                            print(
                                f"  {gm_key_display}. {general_actions_menu[gm_key_display]['text']}"
                            )
                    continue
                else:
